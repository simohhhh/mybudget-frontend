        ...
        <header className="h-20 shrink-0 pl-16 pr-4 md:px-8 flex justify-between items-center bg-[#f4f7fb] dark:bg-slate-900">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <button onClick={() => setIsLangOpen(!isLangOpen)} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm text-slate-700 dark:text-slate-200">
              ...
              {isLangOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-36 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-lg rounded-2xl overflow-hidden z-20 animate-fade-in py-1">
                    {languages.map((lng) => (
                      <button key={lng.code} onClick={() => { i18n.changeLanguage(lng.code); setIsLangOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-2 text-sm font-bold ${currentLangObj.code === lng.code ? 'bg-slate-50 dark:bg-slate-800 text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
            ...
            <div onClick={toggleDarkMode} className="flex items-center gap-3 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm text-slate-600 dark:text-slate-300">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <form onSubmit={handleSauvegarderInfo} className="flex-1 flex flex-col">
                  <div className="space-y-6">
                    <div>
                      ...
                      <input type="text" value={utilisateur.nom} onChange={(e) => setUtilisateur({...utilisateur, nom: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600/50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white font-medium" required />
                    </div>
                    <div>
                  ...
                  <div className="mt-auto pt-8 flex justify-end">
                    <div className="w-full border-t border-slate-100 dark:border-slate-700/50 pt-6 flex justify-end">
                        <button type="submit" disabled={utilisateur.nom === utilisateurInitial.nom && utilisateur.email === utilisateurInitial.email} className="flex items-center gap-2 px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg disabled:opacity-50">
              ...
              <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700/50 flex flex-col">
                ...
                <form onSubmit={handleMiseAJourMotDePasse} className="flex-1 flex flex-col">
                  <div className="mb-6 space-y-6">
                    <div>
                      <div className="flex justify-between items-center mb-2 ml-1">
                        ...
                        <button type="button" onClick={handleMotDePasseOublieDirect} className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700">{t('profile.forgotPassword', 'Oublié ?')}</button>
                      </div>
                      <div className="relative">
                        <input type={afficherActuel ? "text" : "password"} required value={motsDePasse.actuel} onChange={(e) => setMotsDePasse({...motsDePasse, actuel: e.target.value})} className="w-full px-5 py-3.5 pr-12 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600/50 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-slate-800 dark:text-white" />
                        <button type="button" onClick={() => setAfficherActuel(!afficherActuel)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amber-500">{afficherActuel ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                          ...
                          <div className="relative">
                              ...
                              <button type="button" onClick={() => setAfficherNouveau(!afficherNouveau)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amber-500">{afficherNouveau ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                          </div>
                      </div>
                      <div>
                          ...
                          <div className="relative">
                              <input type={afficherConfirmer ? "text" : "password"} required value={motsDePasse.confirmer} onChange={(e) => setMotsDePasse({...motsDePasse, confirmer: e.target.value})} className={`w-full px-5 py-3.5 pr-12 bg-slate-50 dark:bg-slate-900 border rounded-xl outline-none focus:ring-2 text-slate-800 dark:text-white ${motsDePasse.confirmer && motsDePasse.nouveau !== motsDePasse.confirmer ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-200 dark:border-slate-600/50 focus:ring-amber-500'}`} />
                              <button type="button" onClick={() => setAfficherConfirmer(!afficherConfirmer)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amber-500">{afficherConfirmer ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                          </div>
                      </div>
                    </div>
                  </div>
                  <div className="mt-auto pt-8 flex justify-end">
                    <div className="w-full border-t border-slate-100 dark:border-slate-700/50 pt-6 flex justify-end">
                        <button type="submit" disabled={!motsDePasse.actuel || !motsDePasse.nouveau || motsDePasse.nouveau !== motsDePasse.confirmer} className="flex items-center gap-2 px-8 py-3 bg-slate-800 dark:bg-slate-700 text-white font-bold rounded-xl hover:bg-slate-900 dark:hover:bg-slate-600 shadow-lg disabled:opacity-50">
              ...
              <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                    ...
                    <button onClick={() => navigate('/Support')} className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold rounded-xl">{t('profile.contactBtn', 'Contacter')}</button>
                  </div>
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                    ...
                    <button onClick={handleLogout} className="px-5 py-2.5 bg-slate-100 hover:bg-rose-50 dark:bg-slate-700 dark:hover:bg-rose-500/20 text-slate-700 hover:text-rose-600 dark:text-slate-200 dark:hover:text-rose-400 font-bold rounded-xl">{t('profile.quitBtn', 'Quitter')}</button>
                  </div>
              </div>
                
              {/* 4. ZONE DE DANGER */}
              <div className="xl:col-span-2 bg-rose-50 dark:bg-rose-500/10 p-8 rounded-[2rem] border border-rose-200 dark:border-rose-500/20">
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <button onClick={handleDeleteAccount} className="px-6 py-3 bg-rose-600 text-white font-bold rounded-xl hover:bg-rose-700 shadow-lg whitespace-nowrap">{t('profile.deleteAccountBtn', 'Supprimer mon compte')}</button>
                </div>
              </div>
            </div>
          </div>
